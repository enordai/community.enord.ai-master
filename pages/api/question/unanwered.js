...
export default function handler(request, response) {
    if (canContinueAPIRequests(request, response)) {
        ...
        if (
            ...
            database.query(
                'SELECT question.qid, question.uid, question.search, question.title, question.body, question.tags, question.added_at, question.edited_at, question.total_answers, question.comments, question.upvotes, question.bookmarks, question.verified, question.hidden, user.username, user.fullname, user.profile_image FROM users user JOIN questions question ON user.uid=question.uid WHERE total_answers<=0 ORDER BY added_at DESC LIMIT ? OFFSET ?',
                [high, low],
                (err, res) => {
                    if (err) {
